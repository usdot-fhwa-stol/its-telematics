...
export class GetAvailableTopics {
    ...
    async execute(truTopicsMessage: TRUTopicsMessage): Promise<TRUTopicsMessage> {
        console.log('GetAvailableTopics: Starting to fetch available topics');
        ...
        console.log(`GetAvailableTopics: Fetching available topics for unit ${truTopicsMessage.unitId}`);
        
        const result = await this.dataSelectionRepository.getAvailableTopics(truTopicsMessage);
        
        console.log('GetAvailableTopics: Available topics retrieved successfully');
        return result;
    }
}
