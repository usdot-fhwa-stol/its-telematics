...
TRU_INST_RE = re.compile(
    r"^\[(?P<ts>\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}\.\d{3})\]\s+"
    r"(?P<file>[^\s:]+)\s+\(\d+\)\s+-\s+(?P<level>\w+)\s*:\s*(?P<msg>.*)$"
)
RSU_MGMT_SERV_RE = re.compile(
    r"^(?P<ts>\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}\.\d{3})\s+"
    r"(?:\x1b\[\d+m)?(?P<level>\w+)\s*(?:\x1b\[m)?\s+"
    r"\[(?P<thread>[^\]]+)\]\s+(?P<class>[^\s]+)\s+-\s+(?P<msg>.*)$"
)
ANSI_CLEANER_RE = re.compile(r"\x1b\[[0-9;]*m")
INFLUX_KEY_VALUE_RE = re.compile(r'([\w.\[\]-]+)=("(?:[^"\\]|\\.)*"|-?\d+(?:\.\d+)?i?)')
INFLUX_TIME_AND_BYTES_RE = re.compile(r"(\d+)\s+\(bytes:\s*(\d+)\)$")

DROP = {"skipped", "no_match"}
...
def _influx_value_to_num(raw: str) -> Any:
    if raw.startswith('"'):
        return raw[1:-1]
    if raw.endswith("i"):
        return int(raw[:-1])
