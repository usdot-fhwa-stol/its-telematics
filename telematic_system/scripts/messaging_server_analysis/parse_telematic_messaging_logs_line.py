...
def parseInfluxfile(logname):
    ...
    for line in influx_log:
        if (
            "ERROR" in line
            or "failure writing points to database" in line
            or "INFO" not in line
        ):
            continue
        ...
        if len(split_line[0].strip()) == 0:
            continue
        raw_timestamp = (
            split_line[0].strip().split(" ")[0]
            + " "
            + split_line[0].strip().split(" ")[1]
        )

        write_time_datetime_utc = datetime.strptime(
            raw_timestamp, "%Y-%m-%d %H:%M:%S.%f"
        )
        write_time_datetime_utc = write_time_datetime_utc.replace(tzinfo=pytz.UTC)
        local_zone = tz.tzlocal()

        # Convert timezone of log datetime from UTC to local
        write_time_datetime_local = write_time_datetime_utc.astimezone(local_zone)
        ...
        msg_timestamp_in_milli = int(int(split_line_msg.split(" ")[2].strip()) / 1000)
        ...
        writer.writerow(
            [
                topic_name,
                str(datetime.fromtimestamp(msg_timestamp_in_milli / 1000.0)),
                write_time_datetime_local,
                delay,
            ]
        )
